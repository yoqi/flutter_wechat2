...
import 'package:skybot/page/login/controller/register_controller.dart';
...
class RegisterPage extends BaseGetBuilder<RegisterController> {
      ...
      child: Column(
        ...
        children: [
          ...
          _buildOperateLayout(
              Ids.nickname.str(),
              _buildTextField(
                  Ids.nickname_example, controller.nicknameController,
          ...
          _buildOperateLayout(
              ...
              TapWidget(
                  onTap: () async {
                    var zoneCode =
                        await NavigatorUtils.toNamed(ZoneCodePage.routeName);
                    if (zoneCode != null) {
                      controller.changeZoneCode(zoneCode);
                    }
                  },
                  child: Text(
                    '${controller.currZone.name}'
                    '(+${controller.currZone.tel})',
                    style: TextStyle(color: Colours.black, fontSize: 32.sp),
                  ))),
          _buildOperateLayout(
              Ids.phone.str(),
              _buildTextField(Ids.phone_input_hint, controller.phoneController,
                  inputType: TextInputType.phone)),
          _buildOperateLayout(
              Ids.password.str(),
              _buildTextField(Ids.input_password, controller.passwordController,
                  inputType: TextInputType.name)),
          100.sizedBoxH,
          _buildPrivacy(),
          40.sizedBoxH,
  ...
  _buildAvatar(BuildContext context) {
    return TapWidget(
      ...
      child: (controller.avatar == null)
          ? Image.asset(
              Utils.getImgPath('select_avatar',
                  dir: 'avatar', format: Utils.WEBP),
              width: 150.w,
              height: 150.w,
            )
          : Image.file(
  ...
  _buildOperateLayout(String hint, Widget operate) {
    return Container(
      ...
      decoration: BoxDecoration(
          border: Border(
              bottom: BorderSide(
                  width: 1.w, color: Colours.c_999999.withOpacity(0.3)))),
      child: Row(
        children: [
          20.sizedBoxW,
          SizedBox(
      ...
      children: [
        ...
        Text.rich(
          TextSpan(
            children: <TextSpan>[
              TextSpan(
                  text: Ids.software_licensing_and_services_ordinance_1.str(),
                  style: TextStyle(color: Colours.c_666666, fontSize: 28.sp)),
              TextSpan(
                  text: Ids.software_licensing_and_services_ordinance_2.str(),
                  style: TextStyle(color: Colours.c_0066FF, fontSize: 28.sp),
                  recognizer: TapGestureRecognizer()
                    ..onTap = () {
                      NavigatorUtils.toNamed(WebViewPage.routeName,
                          arguments: WebviewArguments(
                              title: Ids
                                  .software_licensing_and_services_ordinance_2
                                  .str(),
                              url: Constant.SOFTWARE_LICENSING));
                    }),
              TextSpan(
                  text: Ids.software_licensing_and_services_ordinance_3.str(),
                  style: TextStyle(color: Colours.c_666666, fontSize: 28.sp)),
            ],
          ),
  ...
  _builcConfirmBtn() {
    ...
    return CommonBtn(
      ...
      textStyle: TextStyle(
          color: enable ? Colours.white : Colours.c_999999, fontSize: 32.sp),
