...
import 'package:skybot/page/util/controller/phone_preview_controller.dart';
import 'package:skybot/widget/base_scaffold.dart';
import 'package:skybot/core.dart';
import 'package:skybot/widget/dialog/dialog_bottom_widget.dart';
...
class PhotoPreviewPage extends BaseGetBuilder<PhonePreviewController> {
  ...
  static open(String url, {String? heroTag}) {
    if (!TextUtil.isEmpty(url)) {
      NavigatorUtils.toNamed(routeName,
          arguments: PhotoPreviewArguments(url: url, heroTag: heroTag));
    }
  }
  ...
  Widget controllerBuilder(
      BuildContext context, PhonePreviewController controller) {
    return MyScaffold(
      showAppbar: false,
            ...
            child: Center(
              child: Hero(
                ...
                child: ExtendedImage(
                  ...
                  fit: BoxFit.fitWidth,
                  image: (!_arguments.url!.contains('http')
                          ? FileImage(File(_arguments.url!))
                          : CachedNetworkImageProvider(_arguments.url!))
