...
import 'package:skybot/page/main/chat/controller/chat_detail_controller.dart';
...
import 'package:skybot/widget/common_switch.dart';
import 'package:skybot/widget/lable_widget.dart';
...
class ChatDetailPage extends BaseGetBuilder<ChatDetailContoller> {
  ...
  Widget controllerBuilder(
      BuildContext context, ChatDetailContoller controller) {
    return MyScaffold(
  ...
  Widget _buildBody(BuildContext context) {
    return Column(
      children: [
        ...
        if (controller.conversation.isGroup) Colours.c_EEEEEE.toLine(10.w),
        if (controller.conversation.isGroup) _buildGroupInfo(),
        Colours.c_EEEEEE.toLine(10.w),
        _buildChatSetting(),
        if (controller.conversation.isGroup) _buildQuit()
      ],
    );
  }
  ...
  Widget _buildAvatarHeader(BuildContext context) {
    ...
    footWidgets.add(TapWidget(
      onTap: () async {
        var usernames = await NavigatorUtils.toNamed(SelectFriendPage.routeName,
            arguments: SelectFriendArguments(
                title: Ids.create_chat.str(),
                selectedUsername: controller.conversation.membersStr));
        if (usernames != null) {
      ...
      child: Column(
        children: [
          Image.asset(
            Utils.getChatImgPath('icon_add_member'),
            width: 100.w,
            height: 100.w,
          ),
          const Spacer()
    ...
    if (controller.conversation.isGroup &&
        controller.conversation.creator == UserController.instance.username) {
      footWidgets.add(Column(
        children: [
          Image.asset(
            Utils.getChatImgPath('icon_min_member'),
            width: 100.w,
            height: 100.w,
          ),
          const Spacer()
    ...
    return Container(
      ...
      child: GridView.builder(
        ...
        itemCount:
            (controller.conversation.members?.length ?? 0) + footWidgets.length,
        shrinkWrap: true,
  ...
  Widget _buildGroupInfo() {
    return Column(
      ...
      children: [
        LableWidget(
          lable: Ids.group_chat_name.str(),
          rightWidget: Text(
            controller.conversation.title(),
            style: TextStyle(color: Colours.c_999999, fontSize: 28.sp),
          ),
          onTap: () async {
            var name = await NavigatorUtils.toNamed(
                ChatGroupEditNamePage.routeName,
        ...
        LableWidget(
          lable: Ids.group_chat_code.str(),
          rightWidget: Image.asset(
            Utils.getImgPath('ic_small_code', dir: Utils.DIR_MINE),
            width: 40.w,
            height: 40.w,
          ),
          onTap: () {
            NavigatorUtils.toNamed(QrcodeGroupChatPage.routeName,
                arguments: controller.conversation);
          },
        ),
        Colours.c_EEEEEE.toLine(1.w),
        LableWidget(
          lable: Ids.group_chat_announcement.str(),
        ),
      ],
    );
  }
  ...
  Widget _buildChatSetting() {
    return Column(
      ...
      children: [
        LableWidget(
            lable: Ids.message_no_disturbing.str(),
            rightWidget: CommonSwitch(
              status: controller.conversation.isMuted,
              onChange: (mute) {
        ...
        Colours.c_EEEEEE.toLine(1.w),
        LableWidget(
          lable: Ids.pin_chat.str(),
          rightWidget: CommonSwitch(
            status: controller.conversation.isPin,
            onChange: (pin) {
      ...
      child: TapWidget(
        onTap: () async {
          var result = await NavigatorUtils.showBottomItemsDialog(
              [DialogBottomWidgetItem(Ids.delete_and_quit.str(), 0)]);
          if (result == 0) {
            controller.quit();
          }
        },
        child: Container(
          ...
          child: Center(
            child: Text(
              Ids.delete_and_quit.str(),
              style: TextStyle(color: Colours.c_E63E24, fontSize: 32.sp),
            ),
          ),
        ),
      ),
