...
import 'package:skybot/utils/permission_utils.dart';
import 'package:skybot/widget/subscription_mixin.dart';
...
class PressRecordWidgetState extends State<PressRecordWidget>
      ...
      () {
        ...
        return GestureDetectorCustomLongTapDuration(
          child: Container(
            ...
            child: Center(
              child: Text(
                (status == VOICE_STATUS_START
                        ? Ids.press_start_voice
                        : status == VOICE_STATUS_END
                            ? Ids.press_end_voice
                            : Ids.press_cancel_voice)
                    .str(),
  ...
  _startRecord() async {
    ...
    mFlutterAudioRecorder2 = FlutterAudioRecorder2(path,
    ...
    periodic(50, (time) {
      ...
      mFlutterAudioRecorder2?.current().then((value) {
        debugPrint(
            'metering ${value?.metering?.peakPower} ${value?.metering?.averagePower}');
