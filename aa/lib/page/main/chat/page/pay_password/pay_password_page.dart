...
class _PayPasswordPageState extends State<PayPasswordPage> {
  ...
  static const FINGERPRINT_STATUS_CHECK = 1;

  ///正在指纹识别
  static const FINGERPRINT_STATUS_ERROR = 2;
  ...
  bool get isfingerprintChecking =>
  ...
  _buildBody(BuildContext context) {
    return Stack(
      children: [
        if (!isfingerprintChecking) _buildCenter(context),
        if (!isfingerprintChecking) _buildKeyboard(),
        if (isfingerprintChecking) _buildFingerprintChecking()
      ],
    );
  }
                ...
                children: [
                  ...
                  Align(
                    ...
                    child: TapWidget(
                        onTap: () {
                          setState(() {
                            payMethod = payMethod == PAY_METHOD_FINGERPRINT
                                ? PAY_METHOD_PASSWORD
                                : PAY_METHOD_FINGERPRINT;
                          });
                        },
                        child: Text(
                          (payMethod == PAY_METHOD_FINGERPRINT)
                              ? Ids.used_fingerprint.str()
                              : Ids.used_password.str(),
                          style: TextStyle(
                              color: Colours.c_5B6B8D, fontSize: 32.sp),
                        )),
                  )
                ],
              ),
            ...
            (payMethod == PAY_METHOD_FINGERPRINT)
                ? SizedBox(
                    ...
                    child: HBPasswordInputTextField(
                      ...
                      length: 6,
                      textStyle: TextStyle(
                          fontSize: 20.sp, color: Colours.theme_color),
                      onChange: (text) {
                        if (text.length == 6) {
                          if (text == '000000') {
            ...
            child: Column(
              ...
              children: [
                ...
                Icon(
                  ...
                  size: 100.w,
                  color: fingerprintChecking == FINGERPRINT_STATUS_CHECK
                      ? Colours.c_999999
                      : Colours.c_E63E24,
                ),
                40.sizedBoxH,
  ...
  _authenticate() async {
    bool didAuthenticate = await LocalAuthentication.authenticate(
      localizedReason: Ids.check_fingerprint.str(),
