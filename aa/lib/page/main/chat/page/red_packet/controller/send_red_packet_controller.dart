...
class SendRedPacketController extends BaseXController {
  ...
  void sendRedPacket() async {
    ...
    try {
      ...
      if (conversation.isGroup &&
          amount < 0.01 * int.parse(countController.text)) {
      ...
      var result = await NavigatorUtils.toNamed(PayPasswordPage.routeName,
          arguments: PayInfoArguments(
              amount: amountController.text, hint: Ids.red_package.str()));
      if (result != null) {
        lcPost(() async {
          ...
          ///红包个数
          lcObject['packet_count'] =
              conversation.isSingle ? 1 : int.parse(countController.text);

          ///红包类型
          lcObject['packet_type'] = conversation.isSingle ? 0 : 1;
          lcObject['user_name'] = UserController.instance.username;
          await lcObject.save();
          var redPacketMessage = RedPacketMessage.from(
              redPacketId: lcObject.objectId!,
              hint: hintController.text.trim().isEmpty
                  ? Ids.red_packet_hint.str()
