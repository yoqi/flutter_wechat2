...
class SendRedPacketPage extends BaseGetBuilder<SendRedPacketController> {
  static const int TYPE_SINGLE = 0;
  ...
  Widget controllerBuilder(
      BuildContext context, SendRedPacketController controller) {
    return MyScaffold(
  ...
  _buildBody(BuildContext context) {
    return Stack(
      children: [_buildInput(), _buildKeyboard()],
    );
  }
      ...
      children: [
        ...
        Container(
          ...
          child: Column(
            ...
            children: [
              if (!isSingleType) _buildGroupHint(),
              if (!isSingleType) _buildRedPacketCount(),
              20.sizedBoxH,
              _buildInputAmount(),
              40.sizedBoxH,
              _buildHint(),
  ...
  _buildGroupHint() {
    return Container(
      ...
      child: Align(
          ...
          child: Text(
            Ids.lucky_red_packet.str(),
            style: TextStyle(color: Colours.c_FA9E3B, fontSize: 28.sp),
          )),
    );
  }
  ...
  _buildRedPacketCount() {
    return Column(
      ...
      children: [
        ...
        Text(
          Ids.group_member_count
              .str()
              .sprint(['${controller.conversation.members?.length ?? 0}']),
    ...
    return Container(
      ...
      child: Row(
        children: [
          Text(
            isSingleType
                ? Ids.single_red_packet_amount.str()
                : Ids.total_amount.str(),
            style: TextStyle(color: Colours.black, fontSize: 32.sp),
          ),
          const Spacer(),
          SizedBox(
  ...
  _buildAmount() {
    ...
    return Center(
      child: Text(
        '$symbol$amount',
        style: TextStyle(
            color: Colours.black, fontSize: 80.sp, fontWeight: FontWeight.bold),
      ),
    );
  }
  ...
  _buildSendBtn() {
    return CommonBtn(
      text: Ids.slip_money_into_red_packet.str(),
      width: 250.w,
      height: 80.w,
      onTap: () {
              ...
              if ('del' == keyEvent.key) {
                ...
                if (keyEvent.key == '.') {
                  if (!controller.amountController.text.contains('.')) {
                    controller.amountController.insertText(
                        (controller.amountController.text.isEmpty ? '0' : '') +
                            keyEvent.key);
                  }
                } else {
