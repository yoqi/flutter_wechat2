...
import 'package:skybot/base/base_getx_refresh.dart';
...
class UniappController extends BaseXRefreshController {
  ...
  void releaseWgtToRunPath(LCObject object) {
    ...
    lcPost(() async {
      var name = file.name;
      var uniappTemporaryDirectory =
          await FileUtils.getUniappTemporaryDirectory();
      ...
      await DownLoadManage().download(file.url, wgtFileName,
          done: (path) async {
        await panelController.open();
