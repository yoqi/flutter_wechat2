...
class SelectLocationController extends BaseMapController
  ...
  void onInit() {
    ...
    sliderAnimation = Tween(begin: Offset.zero, end: const Offset(0, -0.3))
  ...
  void onBMFMapCreated(BMFMapController controller) {
    ...
    dituController?.setMapRegionWillChangeCallback(
        callback: (BMFMapStatus status) {
  ...
  poiNearbySearch() async {
    ...
    if (centerOffset != null) {
      var bmfCoordinate = await dituController?.convertScreenPointToCoordinate(
          BMFPoint(centerOffset.dx, centerOffset.dy));
      BMFPoiNearbySearchOption poiNearbySearchOption = BMFPoiNearbySearchOption(
          keywords: <String>['银行', '酒店', '餐饮', '商场'],
          location: BMFCoordinate(22.613863, 114.039066),
          radius: 10000,
          isRadiusLimit: false,
      ...
      nearbySearch.onGetPoiNearbySearchResult(
        ...
        if (errorCode == BMFSearchErrorCode.NO_ERROR) {
          var bmfPoiInfo = BMFPoiInfo.fromMap(
              {'name': Ids.location.str(), 'pt': bmfCoordinate?.toMap()});
