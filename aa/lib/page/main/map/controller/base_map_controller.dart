...
class BaseMapController extends BaseXController {
  ...
  _onBaiduLocationResult(BaiduLocation result) async {
    ...
    SpUtil.putString(
        Constant.SP_LAST_LOCATION, "${result.latitude},${result.longitude}");
  ...
  BaiduLocationAndroidOption initAndroidOptions() {
    BaiduLocationAndroidOption options = BaiduLocationAndroidOption(
        locationMode:
            BMFLocationMode.hightAccuracy, // 定位模式，可选的模式有高精度、仅设备、仅网络。默认为高精度模式
        ...
        locationPurpose:
            BMFLocationPurpose.signIn, // 可选，设置场景定位参数，包括签到场景、运动场景、出行场景
  ...
  BaiduLocationIOSOption initIOSOptions() {
    BaiduLocationIOSOption options = BaiduLocationIOSOption(
      ...
      activityType: BMFActivityType
          .automotiveNavigation, // 应用位置类型 默认为automotiveNavigation
      ...
      allowsBackgroundLocationUpdates:
          false, //指定是否允许后台定位,允许的话是可以进行后台定位的，但需要项目配置允许后台定位，否则会报错，具体参考开发文档
      distanceFilter: 10,
    ); // 设定定位的最小更新距离
    return options;
  }
  ...
  BMFMapOptions initMapOptions({BMFCoordinate? center}) {
    ...
    if (center == null) {
      String? _loaction =
          SpUtil.getString(Constant.SP_LAST_LOCATION, defValue: null);
