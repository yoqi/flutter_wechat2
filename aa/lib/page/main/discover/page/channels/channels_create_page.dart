...
class ChannelsCreatePage extends BaseGetBuilder<ChannelsCreateController> {
  ...
  Widget controllerBuilder(
      BuildContext context, ChannelsCreateController controller) {
    return MyScaffold(
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            20.sizedBoxH,
            _buildCover(context),
            if (controller.videoFile != null) _buildSelectCover(),
            _buildDescribe(),
            _buildLocation(),
          ],
        ),
    ...
    if (controller.coverFile != null) {
          ...
          Container(
              ...
              child: TapWidget(
                  onTap: () {
                    ...
                    NavigatorUtils.toNamed(ChannelsPreviewPage.routeName,
                        arguments: lcObject);
                  },
                  child: ScaleSizeImageWidget(
                    photoWidth: controller.coverSize?.width.toDouble() ?? 0,
                    photoHeight: controller.coverSize?.height.toDouble() ?? 0,
                    photoUrl: controller.coverFile!.path,
                    tapDetail: false,
                  ))),
          TapWidget(
            onTap: () {
      ...
      return TapWidget(
        onTap: () async {
          ...
          if (file != null) {
            var _video = await NavigatorUtils.toNamed(
                ChannelsVideoEditPage.routeName,
                arguments: file);
            if (_video != null) {
  ...
  _buildSelectCover() {
    return Container(
      ...
      child: TapWidget(
        onTap: () async {
          var cover = await NavigatorUtils.toNamed(
              ChannelsSelectCoverPage.routeName,
              arguments: controller.videoFile);
          if (cover != null) {
      ...
      child: TapWidget(
        ...
        child: Row(
          children: [
            ...
            20.sizedBoxW,
            Text(
              _poiInfo == null ? Ids.the_location.str() : _poiInfo.city ?? '',
              style: TextStyle(
                  color: _poiInfo == null ? Colours.black : Colours.theme_color,
                  fontSize: 28.sp),
            ),
