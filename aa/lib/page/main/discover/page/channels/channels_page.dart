...
class _ChannelsPageState extends State<ChannelsPage>
  ...
  void initState() {
    tabController = TabController(
        vsync: this, length: titles.length, initialIndex: titles.length - 1);
    super.initState();
  }
        ...
        children: [
          ...
          Align(
            ...
            child: IconButton(
              ...
              onPressed: () async {
                var file =
                    await DialogUtil.choosePhotoDialog(context, isVideo: true);
                if (file != null) {
                  ...
                  if (_video != null) {
                    var result = await NavigatorUtils.toNamed(
                        ChannelsCreatePage.routeName,
                        arguments: file);
                    if (result ?? false) {
                      ChannelsController? controller;
                      switch (tabController.index) {
                        case 0:
                          controller = Get.find<ChannelsController>(
                              tag: ChannelsItemPage.tagFollow);
                          break;
                        case 1:
                          controller = Get.find<ChannelsController>(
                              tag: ChannelsItemPage.tagFriend);
                          break;
                        case 2:
                          controller = Get.find<ChannelsController>(
                              tag: ChannelsItemPage.tagRecommend);
                          break;
                      }
