...
import 'package:skybot/widget/load_widget.dart';
import 'package:skybot/widget/remove_top_widget.dart';
...
class ChannelItemWidget extends BaseGetBuilder<ChannelsCommentsController> {
  ...
  ChannelItemWidget({required this.channel, this.channelsController, Key? key})
      : super(key: key);
  ...
  Widget controllerBuilder(
      BuildContext context, ChannelsCommentsController controller) {
  ...
  _buildBottomBar(ChannelsCommentsController controller) {
    ...
    bool _liked = liked.hasIndex(
        (element) => element['username'] == UserController.instance.username);
    bool _favorited = favorited.hasIndex(
        (element) => element['username'] == UserController.instance.username);
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          _buildOperateItem(
              _liked ? 'icon_channels_liked' : 'icon_channels_like',
              liked.length, () async {
            await controller.likeChannels(channel);
            controller.update();
          }, true),
          _buildOperateItem('icon_channels_forword', 0, () {
            Share.share('${channel['video']['url']}',
                subject: Ids.channels.str());
          }, false),
          _buildOperateItem(
              _favorited ? 'icon_channels_favorited' : 'icon_channels_favorite',
              favorited.length, () async {
  ...
  _buildOperateItem(
      String icon, int num, GestureTapCallback onTap, bool aminationLike) {
    if (aminationLike) {
      return Container(
                  ...
                  () => RemoveTopPaddingWidget(
                    child: EasyRefresh(
                      ...
                      onLoad: controller.list.isNotEmpty
                          ...
                          : null,
                      child: PanelListView.builder(
                        itemBuilder: (context, index) {
                          return ChannelsCommentItem(
      ...
      child: Container(
        ...
        child: Row(
          children: [
            ...
            CommonBtn(
              onTap: () async {
                controller.publishComments(
                    channel, controller.textEditingController.text.trim());
              },
              text: Ids.published.str(),
              enable: controller.textEditingController.text.isNotEmpty,
            ),
            20.sizedBoxW,
          ],
        ),
