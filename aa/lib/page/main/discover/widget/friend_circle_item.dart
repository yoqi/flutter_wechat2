...
class FriendCircleItem extends StatefulWidget {
  ...
  FriendCircleItem({required this.lcObject, required this.controller, Key? key})
      : super(key: key);
...
class _FriendCircleItemState extends State<FriendCircleItem> {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(
        ...
        children: [
          AvatarWidget(
              avatar: widget.lcObject['user']['avatar'], weightWidth: 100.w),
          20.sizedBoxW,
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              ...
              children: [
                ...
                _buildLocation(),
                20.sizedBoxH,
                if (widget.lcObject['mediaType'] == 1) _buildImage(),
                if (widget.lcObject['mediaType'] == 2) _buildVideo(),
                20.sizedBoxH,
  ...
  Widget _buildFooder(BuildContext context) {
    return Row(
      ...
      children: [
        Text(
          (widget.lcObject.createdAt!.millisecondsSinceEpoch)
              .commonDateTime(showTime: true),
          style:
              TextStyle(color: Colours.c_999999, fontSize: 24.sp, height: 1.1),
        ),
        10.sizedBoxW,
        if (widget.lcObject['user']['username'] ==
            UserController.instance.username)
          TapWidget(
              onTap: () async {
                var result = await DialogUtil.showConfimDialog(
                    context, Ids.delete.str());
                if (result ?? false) {
              ...
              child: Text(
                Ids.delete.str(),
                style: TextStyle(
                    color: Colours.c_5B6B8D, fontSize: 24.sp, height: 1.1),
              )),
        const Spacer(),
        CustomPopupMenu(
          ...
          menuBuilder: () {
            ...
            bool _liked = liked.hasIndex((element) =>
                element['username'] == UserController.instance.username);
            return Container(
              width: 240.w,
              height: 60.w,
                ...
                children: [
                    ...
                    child: TapWidget(
                      onTap: () async {
                        ...
                        await widget.controller
                            .likeFriendCircle(widget.lcObject);
                        setState(() {});
                      },
                      child: Row(
                        children: [
                          Icon(Icons.favorite_border,
                              color: Colours.white, size: 24.sp),
                          10.sizedBoxW,
                          Text(
                            _liked ? Ids.cancel.str() : Ids.like.str(),
                            style: TextStyle(
                  ...
                  Expanded(
                    child: TapWidget(
                      onTap: () async {
                        _customPopupMenuController.hideMenu();
                        String? comment = await NavigatorUtils.toBottomPage(
                            const FriendCircleCommentDialog());
                        if (comment != null) {
                          await widget.controller
                              .comment(widget.lcObject, comment);
                          setState(() {});
                        }
                      },
                      child: Row(
          ...
          child: Container(
            width: 60.w,
            height: 25.w,
            decoration: Colours.c_EEEEEE.boxDecoration(borderRadius: 4.w),
            child: Center(
              child: Row(
                mainAxisAlignment: MainAxisAlignment.center,
                children: [
                  Container(
                    ...
                    height: 10.w,
                    decoration:
                        Colours.c_5B6B8D.boxDecoration(borderRadius: 10.w),
                  ),
                  10.sizedBoxW,
                  Container(
                    width: 10.w,
  ...
  Widget _buildImage() {
    ...
    if (photos.length == 1) {
      ...
      return ScaleSizeImageWidget(
        photoHeight: photo['height'].toDouble(),
        photoUrl: photo['url'],
        photoWidth: photo['width'].toDouble(),
      );
    }
    return FriendCircleGridView(
        photos: photos.map<String>((e) => e['url']).toList());
  }

  Widget _buildVideo() {
    return ScaleSizeVideoWidget(
      photoWidth: widget.lcObject['thumbnail']['width'].toDouble(),
  ...
  Widget _buildLiked() {
    ...
    return Container(
      ...
      child: Text.rich(
        TextSpan(
          children: <InlineSpan>[
            ImageSpan(AssetImage(Utils.getIconImgPath('icon_liked')),
                imageWidth: 28.sp,
                imageHeight: 28.sp,
                margin: EdgeInsets.only(right: 10.w)),
            ...liked.map((e) => TextSpan(
                text: e['nickname'] +
                    (liked.indexOf(e) != liked.length - 1 ? ',' : ''),
                style: TextStyle(
                    color: Colours.c_5B6B8D, fontSize: 28.sp, height: 1.1),
                      ...
                      TextSpan(
                        children: <InlineSpan>[
                          TextSpan(
                              ...
                              style: TextStyle(
                                  color: Colours.c_5B6B8D,
                                  fontSize: 28.sp,
                                  height: 1.1),
                              recognizer: TapGestureRecognizer()
                                ..onTap = () async {
                                  NavigatorUtils.toNamed(
                                      FriendDetailPage.routeName,
                                      arguments: e['sender']['username']);
                                }),
                          TextSpan(
                              text: '：' + e['comment'],
                              style: TextStyle(
                              ...
                              recognizer: TapGestureRecognizer()
                                ..onTap = () async {
                                  if (e['sender']['username'] ==
                                      UserController.instance.username) {
                                    var _delete =
                                        await DialogUtil.showConfimDialog(
                                            context, Ids.delete_comment.str());
                                    if (_delete ?? false) {
                                      await widget.controller
                                          .deleteComment(widget.lcObject, e);
                                      setState(() {});
                                    }
                                  }
                                }),
  ...
  _buildLocation() {
    ...
    return Container(
      ...
      child: TapWidget(
          onTap: () {
            NavigatorUtils.toNamed(PreviewLocationPage.routeName,
                arguments: poiInfo.pt);
          },
          child: Text(
            poiInfo.name ?? '',
            style: TextStyle(color: Colours.c_5B6B8D, fontSize: 24.sp),
          )),
    );
  }
