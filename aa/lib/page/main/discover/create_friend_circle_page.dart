...
class CreateFriendCirclePage
    extends BaseGetBuilder<CreateFriendCircleController> {
  ...
  @override
  CreateFriendCircleController? getController() =>
      CreateFriendCircleController();

  @override
  Widget controllerBuilder(
      BuildContext context, CreateFriendCircleController controller) {
    bool _enable = controller.textEditingController.text.trim().isNotEmpty ||
        _mediaType != 0;
    return MyScaffold(
    ...
    return Container(
      ...
      child: TextField(
        ...
        decoration: InputDecoration(
          ...
          hintStyle:
              TextStyle(color: Colours.c_999999, fontSize: 32.sp, height: 1.0),
          focusedBorder:
        ...
        onTap: () async {
          ...
          if (selectedAssets != null && selectedAssets!.isNotEmpty) {
            if (selectedAssets!.length == 1) {
              ...
              await Future.forEach<AssetEntity>(selectedAssets!,
                  (element) async {
    ...
    return Container(
      ...
      child: ReorderableItemsView(
        ...
        feedBackWidgetBuilder: (BuildContext context, int index, Widget child) {
          return SizedBox(
              width: _itemHeight, height: _itemHeight, child: child);
        },
        isGrid: true,
        staggeredTiles:
            widgets.map((e) => const StaggeredTile.count(1, 1)).toList(),
        children: widgets,
        ...
        onDragEnd: (index) {
          ...
          var includeWidget =
              _getIndexKey(-1).includeWidget(_getIndexKey(index));
          if (includeWidget) {
          ...
          onTap: () async {
            if (photos.isNotEmpty) {
              var result = await NavigatorUtils.toNamed(
                  VideoPerviewPage.routeName,
                  arguments: VideoArguments(
                      url: photos.first.path,
                      actionType: VideoArguments.actionTypeDelete));
                ...
                child: Stack(
                  children: [
                    ...
                    Center(
                      child: Container(
                          decoration:
                              Colours.white.boxDecoration(borderRadius: 60.w),
                          child: Icon(
                            Icons.play_circle,
  ...
  Widget _buildLocation() {
    ...
    return Container(
      ...
      child: TapWidget(
        onTap: () async {
          var result =
              await NavigatorUtils.toNamed(NearbyLocationPage.routeName);
          if ('remove' == result) {
          } else if (result != null) {
        ...
        child: Row(
          children: [
            Icon(
              ...
              color: _poiInfo == null ? Colours.black : Colours.theme_color,
            ),
            20.sizedBoxW,
            Text(
              _poiInfo == null ? Ids.the_location.str() : _poiInfo.name ?? '',
              style: TextStyle(
                  color: _poiInfo == null ? Colours.black : Colours.theme_color,
                  fontSize: 28.sp),
            )
