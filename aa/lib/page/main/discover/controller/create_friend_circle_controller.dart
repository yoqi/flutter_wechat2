...
import 'package:skybot/utils/image_util.dart';
  ...
  createFriendCircle(int mediaType, List<File> files) {
    lcPost(() async {
      ...
      if (mediaType == CreateFriendCirclePage.mediaTypeImage) {
        ...
        var thumbnailLc =
            await LCFile.fromPath(thumbnailFile.filename, thumbnailFile.path);
        ...
        debugPrint(
            'mediaInfo ${mediaInfo?.width} ${mediaInfo?.height} ${mediaInfo?.orientation}');
        if (mediaInfo != null) {
