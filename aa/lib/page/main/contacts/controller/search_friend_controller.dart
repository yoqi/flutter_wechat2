  ...
  void search() async {
    ...
    if (!TextUtil.isEmpty(string)) {
      lcPost(() async {
        ...
        LCQuery<LCObject> priorityOneOrTwo =
            LCQuery.or([userQueryPhone, userQueryId]);
        priorityOneOrTwo.limit(1);
        List<LCObject?>? results = await priorityOneOrTwo.find();
        if (results?.isNotEmpty ?? false) {
          var result = await NavigatorUtils.toNamed(FriendDetailPage.routeName,
              arguments: results!.first!['username']);
          if (result ?? false) {
