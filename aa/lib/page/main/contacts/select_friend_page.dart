...
class _SelectFriendPageState extends State<SelectFriendPage> {
  ...
  _scrollToTag(String? tag) {
    if (tag != null) {
      ...
      if (_index != -1) {
        _scrollController.scrollToIndex(_index,
            ...
            RemoveTopPaddingWidget(
              ...
              itemBuilder: (context, index) {
                ...
                return AutoScrollTag(
                    ...
                    child: FriendItem(
                      friend: _friend,
                      lastFriend: _friends.safetyItem(index - 1),
                      selectType: (_arguments.selectedUsername
                                  ?.contains(_friend['followee']['username']) ??
                              false)
                          ? -1
                          : _selectedList.contains(_friend)
                              ? 1
                              : 0,
                      onTap: (friend) {
        ...
        child: ListView.builder(
          itemBuilder: (context, index) {
            return Container(
                ...
                height: 60.w,
                child: AvatarWidget(
                    avatar: _selectedList[index]['followee']['avatar'],
                    weightWidth: 60.w));
          },
    ...
    return Align(
      ...
      child: IndexBar(
        ...
        indexHintBuilder: (BuildContext context, String tag) {
          return Text(
            tag,
            style: TextStyle(color: Colours.c_EEEEEE, fontSize: 24.sp),
          );
        },
    ...
    return Container(
      ...
      child: Align(
        ...
        child: CommonBtn(
          ...
          width: 150.w,
          text:
              '${Ids.finish.str()}${_selectedList.isNotEmpty ? '(${_selectedList.length})' : ''}',
          onTap: () {
            NavigatorUtils.pop(_selectedList
                .map<String>((e) => e['followee']['username'])
                .toSet());
          },
          backgroundColor:
              _selectedList.isNotEmpty ? Colours.theme_color : Colours.c_CCCCCC,
          enable: _selectedList.isNotEmpty,
        ),
      ),
    );
  }
