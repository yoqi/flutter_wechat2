...
class ChatManagerController extends BaseXController {
  ...
  void _onUnreadMessageCountUpdated({
    required Client client,
    required Conversation conversation,
  }) {
    debugPrint(
        '_onUnreadMessageCountUpdated ${conversation.unreadMessageCount}');
  ...
  void _onMessageDelivered({
    ...
    debugPrint(
        'onMessageDelivered ${conversation.lastMessage?.contentText} ${messageID}');
  }
  ...
  void _onMessageUpdated({
    ...
    debugPrint(
        'onMessageUpdated ${conversation.lastMessage?.contentText} ${updatedMessage.contentText}');
  }
  ...
  void _onMessage(
    ...
    debugPrint(
        'onMessage ${conversation.unreadMessageCount} ${message.contentText}');
    ...
    if (_currentConversation != null &&
      ...
      NotificationUtil.showNotificationWithNoSound(
          conversation.title(), message.contentText,
  ...
  _updateConversation(Conversation conversation) {
    var indexWhere =
        chatList.indexWhere((element) => element.id == conversation.id);
    if (indexWhere != -1) {
  ...
  _sortChat() {
    chatList.sort((c1, c2) {
      ...
      return (c1.lastMessageTimestamp ?? 0) > (c2.lastMessageTimestamp ?? 0)
          ? -1
          : 1;
    });
  }
  ...
  createSingleChat(String membersId) {
    ...
    if (chat != null) {
      NavigatorUtils.offNamedUntil(ChatPage.routeName, MainPage.routeName,
          arguments: chat.id);
    } else {
      lcPost(() async {
        Conversation conversation = await imClient.createConversation(
            isUnique: true, members: {membersId}, name: 'test');
  ...
  Conversation? getSingleChatInfoOfUsername(String username) {
    var indexWhere = chatList.indexWhere((element) =>
        element.isSingle &&
        (element.members?.contains(username) ?? false) &&
        (element.members?.contains(UserController.instance.username) ?? false));
    if (indexWhere != -1) {
      return chatList[indexWhere];
    }
    return null;
  ...
  createConversation(Set<String> members) async {
    lcPost(() async {
      Conversation _c = await imClient.createConversation(
          members: members, name: Ids.group_chat.str(), isUnique: false);
