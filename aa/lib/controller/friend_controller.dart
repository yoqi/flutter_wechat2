...
class FriendController extends BaseXController {
  ...
  friendIndex() {
    lcPost(
      () async {
        LCQuery<LCObject> query = UserController.instance.user!
            .followeeQuery()
        ...
        if (result?.isNotEmpty ?? false) {
          ...
          for (var element in friends) {
            String pinyin =
                PinyinHelper.getPinyinE(element['followee']['nickname'])
                    .substring(0, 1)
                    .toLowerCase();
  ...
  isFriend(String username) {
    return friends.indexWhere(
            (element) => element['followee']['username'] == username) !=
        -1;
  }
}
