...
class MapUtil {
  ...
  static Future<void> showDirections(longitude, latitude) async {
    ...
    var index = await NavigatorUtils.showBottomItemsDialog(list
        .map<DialogBottomWidgetItem>(
            (e) => DialogBottomWidgetItem(e.mapName, e.mapType.index))
        .toList());
    if (index != null) {
      var item = list.itemWhere((element) => element.mapType.index == index);
      MapLauncher.showDirections(
          mapType: item!.mapType, destination: Coords(latitude, longitude));
    }
  }
}
