...
class PatternUtil {
    ...
    if (hasMatch) {
      ...
      for (int i = 0; i < _allMatches.length; i++) {
        ...
        if (i == 0 && element.start != 0) {
          ...
          stack.add(TextSpan(
              children: _checkUrlPattern(text, textStyle, onURLClick)));
        }
        ...
        if (emoji != null) {
          stack.add(ImageSpan(
              AssetImage(Utils.getImgPath(emoji.name!, dir: Utils.DIR_EMOJI)),
              imageWidth: imageSize.w,
              imageHeight: imageSize.w));
        } else {
          stack.add(TextSpan(
              children: _checkUrlPattern(group!, textStyle, onURLClick)));
        }
      ...
      return RichText(
        text: TextSpan(
            children: _checkUrlPattern(content, textStyle, onURLClick)),
        overflow: overflow,
        maxLines: maxLines,
      );
    }
  ...
  static RichText transformAtStr(String content, TextStyle textStyle,
      {overflow = TextOverflow.visible, int? maxLines}) {
    return RichText(
      text: TextSpan(
        text: content,
        style: textStyle,
  ...
  static RichText checkEmoji(String content, TextStyle textStyle,
      {OnURLClick? onURLClick, int? maxline}) {
    return RichText(
      text:
          TextSpan(children: _checkUrlPattern(content, textStyle, onURLClick)),
      maxLines: maxline,
      overflow: TextOverflow.ellipsis,
    );
  }

  static List<InlineSpan> _checkUrlPattern(
      String content, TextStyle textStyle, OnURLClick? onURLClick) {
    final List<InlineSpan> stack = [];

    final hasMatch = _urlPattern.hasMatch(content);
    if (hasMatch) {
      final List<RegExpMatch> allMatches =
          _urlPattern.allMatches(content).toList();
            ...
            recognizer: TapGestureRecognizer()
              ..onTap = () async {
                NavigatorUtils.toNamed(WebViewPage.routeName,
                    arguments: WebviewArguments(url: group!));
