...
import 'package:wecha_camera_picker/wechat_camera_picker.dart';
...
class DialogUtil {
  static showLoading({String? msg}) {
    SmartDialog.showLoading(msg: msg ?? Ids.waiting.str(), backDismiss: false);
  }
          ...
          return AlertDialog(
            ...
            actions: <Widget>[
              TextButton(
                child: Text(
                  Ids.cancel.str(),
                  style: const TextStyle(color: Colours.c_E9465D),
                ),
                onPressed: () {
  ...
  static Future<File?> choosePhotoDialog(BuildContext context,
      {bool crop = false,
      double aspectRatio = 1.0,
      bool compress = true,
      bool isVideo = false}) async {
    var result = await NavigatorUtils.showBottomItemsDialog([
      DialogBottomWidgetItem(Ids.shoot.str(), 1),
      DialogBottomWidgetItem(Ids.album.str(), 0)
    ]);
    File? file;
    switch (result) {
      case 0:
      ...
      case 1:
        final AssetEntity? entity = await CameraPicker.pickFromCamera(context,
            pickerConfig: CameraPickerConfig(
                enableRecording: isVideo, onlyEnableRecording: isVideo));
        if (entity != null) {
    ...
    if (file != null && crop) {
      file = await NavigatorUtils.toNamed(CropImagePage.routeName,
          arguments: CropArguments(file: file, aspectRatio: 1.00));
      if (file != null) {
