...
extension KeyExt on GlobalKey {
  ...
  Offset? location({bool hasOffset = true}) {
    final RenderBox? renderBox =
        currentContext?.findRenderObject() as RenderBox?;
    final Offset? offset = renderBox?.localToGlobal(hasOffset
  ...
  bool includeWidget(GlobalKey otherGlobalKey) {
    RenderBox otherRenderBox =
        otherGlobalKey.currentContext?.findRenderObject() as RenderBox;

    final Offset otherOffset = otherRenderBox.localToGlobal(
        Offset(otherRenderBox.size.height / 2, otherRenderBox.size.height / 2));
