...
class ScaleSizeImageWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    Widget image = CacheImageWidget(
      url: photoUrl,
      weightWidth: photoWidth,
      weightHeight: photoHeight,
      hero: true,
    );
    ...
    if (tapDetail) {
      return TapWidget(
          onTap: () {
            NavigatorUtils.toNamed(PhotoPreviewPage.routeName,
                arguments:
                    PhotoPreviewArguments(heroTag: photoUrl, url: photoUrl));
          },
          child: image);
    } else {
      return image;
